...
# Stream screenshot directly via exec-out (avoids device temp file)
$ADB_CMD exec-out screencap -p > "$LOCAL_PATH" 2>/dev/null

# Verify file was created and has content
if [ ! -s "$LOCAL_PATH" ]; then
    # Fallback: classic method with device temp file
